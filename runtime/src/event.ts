import type { Metadata } from "@tivet-gg/actor-core";
import type * as wsToClient from "@tivet-gg/actor-protocol/ws/to_client";
import * as wsToServer from "@tivet-gg/actor-protocol/ws/to_server";
...
export async function handleMessageEvent<A extends AnyActor>(
	...
	try {
		...
		if (error instanceof errors.ActorError && error.public) {
			...
			metadata = {
				url: `https://hub.tivet.gg/projects/${actorMetadata.project.slug}/environments/${actorMetadata.environment.slug}/actors?actorId=${actorMetadata.actor.id}`,
